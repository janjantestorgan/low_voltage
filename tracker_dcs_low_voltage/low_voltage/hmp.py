...
class HMP(object):
    ...
    def command(self, topic: str, message: Union[bytes, float, int]) -> None:
        ...
        if command == 'switch':
            ...
            print(f"CMD for activating the output of the selected channel on LV : {cmd}") 
    ...
    def status(self):
        ...
        for ch in range(self.num_of_channels-1)  :
            ...
            self.HMP4040.write("MEAS:VOLT?")    #Queries the voltage value of the selected channel.
            V = self.HMP4040.read()
            print("V : ",V)
            self.channels[ch+1].vreq = float(V)
        
            self.HMP4040.write("MEAS:CURR?")    #Queries the current value of the selected channel.
            I = self.HMP4040.read()
            print("I : ",I)
